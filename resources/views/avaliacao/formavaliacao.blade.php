...
<div class="flex justify-center">
    <div class="p-6 bg-white shadow sm:rounded-lg w-full max-w-2xl">
        ...
        <form action="{{ !empty($dado->id) ? route('avaliacao.update', $dado->id) : route('avaliacao.store') }}" method="post" class="space-y-4">
            @csrf
            @if (!empty($dado->id))
                @method('PUT')
            @endif
            ...
            <div>
                ...
                <input type="number" min="0" max="10" name="nota" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full" value="{{ old('nota', $dado->nota ?? '') }}">
            </div>

            <div>
                <label for="comentario" class="block font-medium text-sm text-gray-700 mb-1">Comentário</label>
                <input type="text" name="comentario" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full" value="{{ old('comentario', $dado->comentario ?? '') }}">
            </div>
